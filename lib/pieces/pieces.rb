...
class Pieces
  attr_reader :pieces_location_white, :pieces_white, :pieces_location_black, :pieces_black
  ...
  def piece_type_from_location(loc, colour)
    case colour
    when 'White'
      @pieces_location_white[loc]
    when 'Black'
      @pieces_location_black[loc]
    end
  end
  ...
  def location_king(colour)
    case colour
    when 'White'
      @pieces_location_white.key('K')
    when 'Black'
      @pieces_location_black.key('K')
    end
  end
  ...
  def king_valid_moves(location)
    ...
    if @horz_location != 7 && @horz_location != 0
      @allowed_horz_location = [@letters[@horz_location + 1], @letters[@horz_location - 1],
                                @letters[@horz_location]]
    elsif @horz_location == 7
      @allowed_horz_location = %w[G H]
    elsif @horz_location.zero?
      @allowed_horz_location = %w[B A]
    end
